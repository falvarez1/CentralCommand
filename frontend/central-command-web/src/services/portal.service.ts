...
export class PortalService {
  ...
  async getPortals(params?: {
    ...
    const { data } = await apiClient.get<PagedResult<PortalResponse>>(this.basePath, { params });
    return data;
  }

  async getPortalById(id: string): Promise<PortalResponse> {
    const { data } = await apiClient.get<PortalResponse>(`${this.basePath}/${id}`);
    return data;
  }

  async createPortal(portal: PortalCreateRequest): Promise<PortalResponse> {
    const { data } = await apiClient.post<PortalResponse>(this.basePath, portal);
    return data;
  }

  async updatePortal(id: string, portal: PortalUpdateRequest): Promise<PortalResponse> {
    const { data } = await apiClient.put<PortalResponse>(`${this.basePath}/${id}`, portal);
    return data;
  }
  ...
  async checkHealth(id: string): Promise<HealthCheckResponse> {
    const { data } = await apiClient.post<HealthCheckResponse>(
  ...
  async getMetrics(id: string): Promise<PortalMetricsResponse> {
    const { data } = await apiClient.get<PortalMetricsResponse>(
  ...
  async getFavorites(): Promise<PortalResponse[]> {
    const { data } = await apiClient.get<PortalResponse[]>(`${this.basePath}/favorites`);
    return data;
  }

  async toggleFavorite(id: string): Promise<void> {
    await apiClient.post(`${this.basePath}/${id}/favorite`);
  }

  async getRelatedIncidents(id: string): Promise<any[]> {
    const { data } = await apiClient.get(`${this.basePath}/${id}/incidents`);
    return data;
  }
}
